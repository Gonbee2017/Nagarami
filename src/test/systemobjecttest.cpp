...
TEST(systemobject,timeBeginPeriod)
{
    {
        logger lg;
        lg.setPutWithResult
        ...
        (
            describe("timeBeginPeriod failed.(",TIMERR_NOCANDO,")"),
            nm::timeBeginPeriod(1)
        );
        CHECK_EQUAL(1,lg.history().size());
...
TEST(systemobject,timeGetDevCaps)
{
    {
        logger lg;
        lg.setPutWithResult
        ...
        CHECK_THROWS_RUNTIME_ERROR
        (
            describe("timeGetDevCaps failed.(",TIMERR_STRUCT,")"),
...
TEST(systemobject,timeSetEvent)
{
    {
        logger lg;
        ...
        CHECK_THROWS_RUNTIME_ERROR
        (
            describe("timeSetEvent failed."),
            nm::timeSetEvent(1,2,(LPTIMECALLBACK)0x10,3,4)
        );
        CHECK_EQUAL(1,lg.history().size());
        CHECK_EQUAL
        (
